...
def test_unit_economics_calculator_uses_anchor_for_legacy_scheme_label(
    tmp_path: Path,
) -> None:
    client = make_client(tmp_path)
    with client.app.state.session_factory() as db:
        ...
        row = db.scalar(
            select(repository.ReportUnitRow).where(
                ...
                repository.ReportUnitRow.row_uid == "unit-1",
            )
        )
        assert report is not None
        assert row is not None
        row.scheme = "Продажи со склада WB"
        db.flush()
    ...
    assert context["fact"]["scheme"] == "Продажи со склада WB"
    assert context["defaults"]["price_before_spp"] is not None
    assert context["defaults"]["unit_cost"] is not None
...
def test_unit_economics_calculator_is_staff_flagged_and_read_only(
    ...
    assert uvloop.run(disabled_request()) == 404
    ...
    response, me_response = uvloop.run(enabled_requests())
    assert response.status_code == 200
    payload = response.json()
    assert payload["resultKind"] == "estimate"
    assert payload["calculationStatus"] == "ready"
...
def test_unit_economics_calculator_reports_missing_cost_and_bad_scope(
    ...
    missing, wrong_row, wrong_period = uvloop.run(requests())
...
def test_unit_economics_calculator_client_flag_and_target_price(
    ...
    status_code, enabled_for_client = uvloop.run(client_requests())
    ...
    response, me_response = uvloop.run(enabled_client_requests())
    assert response.status_code == 200
    payload = response.json()
    assert payload["calculationStatus"] == "ready"
    assert payload["targetPrice"] is not None
...
def test_cabinet_shell_serves_login_without_report_data(tmp_path: Path) -> None:
    ...
    assert "Ozon + 1C" in cabinet.text
    assert "styles.css?v=20260803-ai-chat-v265" in cabinet.text
    assert "app.js?v=20260803-ai-chat-v265" in cabinet.text
...
def test_cabinet_static_assets_use_readiness_api_and_safe_rendering(
    ...
    assert "20260803-ai-chat-v265" in cabinet.text
