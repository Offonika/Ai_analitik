    ...
    assert logistics["operationalDocs"] == [
        {
            ...
            "summary": (
                "Operational state WB-логистики: test staff-only, client flags "
                "выключены, production-логистика не включена; перед rollout "
                "обязательна свежая runtime/DB проверка."
            ),
        }
    ]
