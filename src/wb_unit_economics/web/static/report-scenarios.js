(() => {
  ...
  function table(rows, options = {}) {
    ...
    const mobileColumns = Array.isArray(options.mobileColumns) && options.mobileColumns.length
      ...
      : columns;
    const label = options.ariaLabel || options.caption || options.label || "Таблица данных";
    const wrapper = node(
      "div",
    ...
    else wrapper.setAttribute("aria-label", label);
    const element = node(
      "table",
    ...
    columns.forEach((column) => {
      const header = node("th", column.className || "", column.label || column.key);
      header.scope = "col";
    ...
    source.forEach((row) => {
      ...
      columns.forEach((column) => {
        ...
        const cell = node("td", column.className || "", value(formatted, "—"));
