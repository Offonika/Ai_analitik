...
нормализует строки в плоский prework-слой. Разрешённая связь с Finance — только
точный `(wb_cabinet_id, srid, nm_id)` после зарегистрированного verified
snapshot; `orderId`, `srid` или товар отдельно не являются fallback.
