...
read-only API, staff-only экран, детерминированные рекомендации и безопасный
агрегатный контекст для AI. Контексты `wb-logistics-v1`–`wb-logistics-v3`,
несовместимая версия ключа и старый отчет без контекста возвращают
`needs_rebuild`.
...
`WP-4` нельзя начинать до нового read-only test-снимка и полного прохождения
gate. Остальные бизнес-решения остаются принятыми.
...
- 2026-07-17 — согласованы правила реального WB-снимка: варианты FBW/FBS
  ...
  также уточнена граница с методикой закрывающей недели основного P&L.
