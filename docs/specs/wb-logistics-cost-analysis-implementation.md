...
32. на ширине 390 px остаются доступны все значения глобального среза, а каждая
    ...
    действия.
33. для `consultant/admin` состояние `needs_rebuild`/`blocked` предлагает
    `Открыть готовую ревизию`, если более новый `ready` report run уже входит в
    серверно отфильтрованный список отчётов текущего клиента; действие меняет
    query-string `report_id`, остаётся в `#tables/logistics` и загружает только
    выбранную immutable-ревизию. Произвольный идентификатор, другой tenant и
    скрытый client-role draft не используются; при отсутствии `ready`-кандидата
    сохраняется прежнее объяснение необходимости новой сборки.
