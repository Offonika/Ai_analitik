...
### Управленческая визуализация и группировка факторов

Основной экран показывает показатели периода, главный вывод, не более трёх
приоритетных действий, состав расходов, недельную динамику и рейтинг товаров.
Габариты, контрольные замеры, тарифы, маршруты и причины возвратов по умолчанию
свёрнуты в disclosure-блоки. Переход из рекомендации раскрывает нужный блок,
прокручивает его в область просмотра и переводит фокус на заголовок.

Группировка выполняется read-only запросом по полному отфильтрованному срезу до
pagination и не меняет immutable mart:

- габариты — одна строка на точную карточку кабинета WB независимо от числа
  организаций и схем отчёта;
- контрольные замеры — одна группа на карточку и московскую дату события;
- причины возвратов — одна группа на канонический товар и дату возврата.

Разные варианты с одинаковым названием не объединяются. Пользователь видит
название и артикул продавца, число карточек/событий/возвратов и точную русскую
причину ограничения. Raw IDs, hashes, пути и source amounts в grouped-ответах
не возвращаются. Полностью пустая колонка заменяется одним объяснением над
списком, а одинаковое предупреждение не повторяется в каждой строке.

Factor API сохраняет legacy `view=raw` и additive принимает:

- `view=summary` — coverage, рекомендации и headline без строк;
- `view=grouped` — группы с серверной пагинацией;
- `view=grouped&groupOffset=N` — безопасные бизнес-поля одной раскрытой
  группы, вычисленной в том же неизменяемом срезе и порядке.

Синтетический target полного сценария:
[`docs/design/wb-logistics-management-target.html`](../design/wb-logistics-management-target.html).
...
- 2026-07-26 — принят disclosure-first контракт всего логистического экрана:
  server-side summary/grouped/raw представления, группировка карточек, замеров
  и возвратов по полному filtered slice, точные русские ограничения и
  business-safe детали без изменения immutable marts. Refresh, report rebuild,
  migration, client/production rollout не входят.
