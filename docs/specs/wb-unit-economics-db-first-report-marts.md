...
truth_priority: 100
related_code: [src/wb_unit_economics/report_marts.py, src/wb_unit_economics/report_exports.py, src/wb_unit_economics/web/models.py, src/wb_unit_economics/web/repository.py, src/wb_unit_economics/web/source_refresh.py, scripts/rebuild_report_from_sources.py, scripts/export_report_artifacts.py]
...
Количество и себестоимость 1С в этой витрине берутся только из строк валовой
прибыли 1С, сопоставленных с выбранными WB-документами. Расходы МП 1С берутся
из `rwb_total` соответствующего месяца ОПиУ. Строки входящих счетов и услуг 1С
сохраняются отдельной расшифровкой `marketplaceServiceRows` и не подменяют
контроль ОПиУ.
