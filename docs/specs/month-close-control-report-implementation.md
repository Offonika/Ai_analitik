...
Автоматическая запись карточки или стадии в Bitrix не входит в эту report spec.
До отдельной реализации связь может фиксироваться вручную безопасным
идентификатором или ссылкой без raw client data.
