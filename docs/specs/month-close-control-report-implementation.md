...
- налоговые факты подтверждает бухгалтер, клиентский вывод утверждает
  консультант;
...
- `accountant` подтверждает фактические налоговые показатели и объясняет
  расхождения, которые нельзя доказать read-only источником.
- `consultant` проверяет coverage, формирует дозапросы и утверждает текст
  клиентского вывода.
...
- Бухгалтерское подтверждение и клиентский вывод имеют разных владельцев.
