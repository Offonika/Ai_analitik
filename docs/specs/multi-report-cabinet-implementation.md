...
- Для бухгалтерского вида действие скачивания явно показывает текущую ревизию
  контракта и время формирования. Имя Excel содержит суффикс ревизии (`vN`),
  чтобы новая загрузка не была неотличима от сохраненной ранее версии файла.
- Бухгалтерские виды используют пошаговый staff-master: существующая ревизия,
  организация и месяц нового запуска, progress worker и новый Excel по точному
  `report_id` показаны раздельно. Общая кнопка download для бухгалтерского вида
  скрыта; обе ссылки доступны внутри мастера с явными подписями.
...
- AI и экспорт используют тот же `report_id` и `report_kind`, который видит
  пользователь.
- Для бухгалтерского Excel версия видна до скачивания и в имени файла; две
  ревизии одного месяца нельзя спутать по одинаковому имени в папке загрузок.
- Мастер бухгалтерского отчёта не заменяет ссылку существующей ревизии ссылкой
  нового запуска до явного reset/reopen и открывает результат по созданному
  `report_id`.
...
- имя бухгалтерского Excel и web-действие скачивания содержат ревизию контракта;
- web-master бухгалтерского вида проверяет существующую ревизию, обязательные
  organization/month, стадии generation и result download по новому report id;
...
- 2026-07-22: бухгалтерский download показывает ревизию и время формирования,
  а имя Excel содержит `vN`, чтобы браузерная папка загрузок не маскировала
  новый отчет старым файлом с тем же именем.
