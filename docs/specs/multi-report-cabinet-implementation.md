...
ответа создается отдельная follow-up-задача.

Прямая запись карточек, стадий или задач в Bitrix не входит в реализацию этой
spec и требует отдельного rollout по принятой ONEPAGE-концепции.
...
- 2026-07-16: принята ONEPAGE-модель внешнего смарт-процесса: одна карточка на
  клиента, организацию и месяц, две связанные задачи, один ответственный
  специалист в первом rollout, ручная отправка `tax_load` и неблокирующий
  ответ клиента. Автоматическая запись в Bitrix и клиентская публикация из
  кабинета остаются отдельными rollout-решениями.
