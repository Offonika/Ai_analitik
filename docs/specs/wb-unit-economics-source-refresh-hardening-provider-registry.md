...
Production weekly/full запускается во вторник в `09:15 MSK`. Daily timer
сохраняет hourly `*:15` внутри окна `08:15-22:15`, но исключает вторник
`09:15`, поэтому два scheduler entry не конкурируют за один tenant; следующий
daily остается во вторник `10:15`. Окно и перенос weekly обоснованы измеренной
доступностью 1С: ночные запуски `00:00-06:00` регулярно возвращали
`onec_odata_metadata_unavailable`, дневные `10:00-22:00` за месяц наблюдений ни
разу не падали. Точные числа — в `docs/runbooks/source-refresh-schedule.md`.
...
- 2026-08-08: daily timer ограничен окном `08:15-22:15`, weekly перенесён на
  ...
  они приходились на `23:00-09:00`, тогда как `10:00-22:00` не дали ни одного
  отказа.
