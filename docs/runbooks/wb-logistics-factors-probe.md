...
| Источник | Scope | HTTP | Вывод |
|---|---|---|---|
| Тарифы box | Тарифы | 200 | `boxDeliveryBase`, `boxDeliveryCoefExpr`, `warehouseName`, периоды `dtNextBox`/`dtTillMax` есть (99 складов). F-2 разблокирован. |
| Тарифы pallet | Тарифы | 200 | `palletDeliveryExpr`, `warehouseName`, `dtNextPallet`/`dtTillMax` есть (100). |
| goods-return | Аналитика | 200 | `reason`, `srid`, `nmId`, `status`, `returnType` есть. Причины возврата продавцу доступны. |
| statistics `supplier/sales` | Статистика | 401 | Токен без scope «Статистика». Склад/направление недоступны. |
| claims | Возвраты покупателями | 401 | Токен без scope «Возвраты покупателями». Комментарий покупателя недоступен. |
| Габариты (Content) | Контент | — | Уже реализовано (F-1), данные в сохранённых карточках. |

Действие на стороне клиента: добавить у WB-токена scope **«Статистика»** и
**«Возвраты покупателями»** (или выпустить ключ с ними) — иначе склад/маршруты
(F-3) и заявки покупателей остаются недоступны. После добавления перезапустить
probe и обновить матрицу.

Готовы к реализации без ожидания: тарифы (F-2) и причины возврата продавца
(goods-return). Заблокированы на scope: F-3 (склад/направление) и claims.
