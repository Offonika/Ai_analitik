...
# Подготовка closed-period insight к staff-only test — 25 июля 2026 года

После объединения prerequisite-исправления создана отдельная ветка от
актуального `origin/main`; работа не смешивается с прежними PR. Spec-first
принят presentation-layer контракт `wb-logistics-insight-v1`:

- основные финансовые KPI, товары, факторы и AI используют только полные
  недели понедельник–воскресенье внутри выбранного периода;
- неполные границы возвращаются и показываются отдельно как фактическая
  логистика без выручки, доли и влияния на прибыль;
- один детерминированный insight используется web, staff AI и аналитическим
  Markdown/DOCX; контракт документа повышен до v4;
- `partial` F-1…F-5 остаётся явным ограничением и не создаёт оценку экономии;
- смена report очищает сохранённое состояние логистики до получения нового
  summary, поэтому старый `needs_rebuild` не переносится на другой report.

Это описание change set, а не evidence развернутой среды. До зелёных
`quality/tests`, immutable runtime и повторной проверки runtime/DB/flags
переключение test не выполняется. DB migration, source refresh, backup,
пересборка Excel, client/production flags и внешние write-операции в пакет не
входят.

Локальные проверки этого пакета ограничиваются релевантным набором. Если
изолированный sandbox блокирует `TestClient` на login, после одного bounded
diagnostic с `faulthandler` тот же точный тест запускается в разрешённом
test-runner вне sandbox; многоминутное слепое ожидание не повторяется. Любая
другая подготовительная операция дольше 15 минут сначала останавливается и
согласовывается.

# Следующий этап

1. Завершить CI и выполнить только staff acceptance closed-period insight на
   test; до этого раздел не считать развернутым.
