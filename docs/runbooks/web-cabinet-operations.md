...
## Пропорциональность перед публикацией отчета

Для обычной публикации уже собранного immutable draft на test новый полный
PostgreSQL backup не выполняется, если одновременно подтверждены:

1. draft единственный в ожидаемом scope, его Excel имеет `ready` и существует;
2. publication blockers отсутствуют, активного source refresh нет;
3. публикация атомарна, previous current и его immutable rows сохраняются;
4. schema, raw snapshots, source lineage и исторические report rows не
   переписываются и не удаляются.

В этом случае rollback-контроль ограничивается безопасной preflight-проверкой,
фиксацией previous/current только в защищенном transient operational state,
атомарной публикацией и немедленным DB/API/browser smoke. Реальные identifiers
не переносятся в Git/Markdown. Нормальный ориентир операции — 5–10 минут.

Новая полностью верифицированная off-host копия обязательна только перед:

- schema migration;
- массовым rewrite/delete;
- raw-row prune/repack;
- другой деструктивной maintenance-операцией;
- production-изменением без доказанного более дешевого rollback;
- явным требованием пользователя.

Если подходящая свежая верифицированная копия уже существует, сначала
проверяется возможность использовать ее, а не создается дубликат. Любой
подготовительный шаг с ожидаемой длительностью больше 15 минут или заметно
дольше основной операции сначала объясняется пользователю и запускается только
после его подтверждения.

## Полный PostgreSQL backup
