...
- 2026-07-17: v2.53 исправил представление бухгалтерского `tax_load`: название
  организации и статусы локализованы, суммы и даты отформатированы, счетчик
  `Проверки` синхронизирован с issues, повторяющиеся действия в обзоре скрыты,
  таблицы доступны на мобильном и с клавиатуры, инструкция стала
  report-kind-specific, общая шапка больше не дублирует scenario, меню не
  обрезает основное действие и добавлена явная favicon.
- 2026-07-16: v2.52 сохранил полный контекст прямой ссылки (`report_kind`,
  организация и месяц) во время первичного восстановления клиента, чтобы
  ссылка на бухгалтерский отчёт не открывала вместо него основную витрину.
- 2026-07-16: v2.51 восстановил `Проверки` для бухгалтерских сценариев,
  добавил понятное объяснение пустого коэффициента ФНС и разрешил test-only
  наполнение информационного графика налоговой нагрузки из локального
  черновика только как `partial_source`, без подмены подтверждённых платежей.
- 2026-07-16: v2.50 устранил подмену выбранного черновика текущим опубликованным
  Excel при скачивании. Защищённый export теперь всегда использует точный
  `report_id`, включает выбранный период в имени файла, а основные даты листа
  `Юнит экономика` записываются как даты Excel в формате `ДД.ММ.ГГГГ`, а не как
  ISO-текст.
