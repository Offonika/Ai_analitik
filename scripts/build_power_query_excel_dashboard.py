...
def _write_dashboard(
    ...
    kpi_cards = [
        ...
        (
            ...
            kpi.get("Маржинальный доход WB после налогов"),
            "rub",
        ),
        ("Маржа WB после налогов", kpi.get("Маржа WB после налогов"), "pct"),
    ...
    _write_table(
        ws,
        "A12",
        ["Месяц", "Выручка после СПП", "Расходы WB", "Маржинальный доход", "Маржа"],
        monthly_small,
        styles,
    ...
    _write_table(
        ws,
        "G12",
        ["Товар", "Артикул 1С", "Баркод", "Выручка", "Маржинальный доход", "Причина"],
        [
            [
                row["Товар"],
                row["Артикул 1С"],
                ...
                row["Маржинальный доход WB после налогов"],
                row["Причина"],
            ]
            for row in top_loss
        ],
...
def _top_loss_rows(rows: list[dict[str, Any]], *, limit: int) -> list[dict[str, Any]]:
    ...
    losses = [
        ...
        if row["Маржинальный доход WB после налогов"] < 0
    ]
    return sorted(losses, key=lambda row: row["Маржинальный доход WB после налогов"])[
        :limit
    ]
